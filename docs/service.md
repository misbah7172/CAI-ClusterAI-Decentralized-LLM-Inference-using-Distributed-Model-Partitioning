# CAI — Service Overview

## What CAI Can Do

CAI is a **distributed AI inference platform** that lets you run large language models across a cluster of low-end PCs using Kubernetes. Here's what it can do:
...
- **conductor** (`cai_cli.py`) - Main command router for run, benchmark, deploy, tune, and analysis flows.
- **launcher** (`cai_cli_dashboard.py`) - Starts the recommended dashboard runtime profile.
...
- **Single command**: `python cai_cli.py run --model microsoft/phi-2 --prompt "Hello" --max-tokens 100 --stream`
...
- **CLI**: `python cai_cli.py hybrid --model <model> --prompt "text" --mode auto`
...
- **CLI**: `python cai_cli.py placement --model <model> --objective balanced`
...
- **CLI**: `python cai_cli.py energy-loop --power-target 100 --latency-target 50 --daemon`
...
- **CLI**: `python cai_cli.py speculative --model <model> --prompt "text" --speculation-length 5`
...
- **CLI**: `python cai_cli.py fault-tolerant --model <model> --prompt "text"`
...
- **CLI**: `python cai_cli.py autotune --model <model> --objective energy --max-trials 20`
...
- **CLI**: `python cai_cli.py plugins --action list`
...
- **CLI**: `python cai_cli.py fcim --report`
...
- **CLI**: `python cai_cli.py adsa --policy adaptive --num-tasks 20 --show-metrics`
...
- **CLI**: `python cai_cli.py active-inference --show-beliefs`
...
- **CLI**: `python cai_cli.py batch --max-batch-size 8 --strategy adaptive --show-status`
...
- **CLI**: `python cai_cli.py dfs-scheduler --pruning bound --num-tasks 10 --num-workers 5`
...
- **CLI**: `python cai_cli.py ilp-scheduler --algorithm auto --num-tasks 20`
...
- **CLI**: `python cai_cli.py onnx --model <model> --output model.onnx --optimize`
...
- **CLI**: `python cai_cli.py simulate --model <model> --optimization-level 2 --approximate-decode`

---

## Energy-Efficient Inference (Core Value)

A key advantage of CAI is **reducing power consumption while producing identical output**.
...
| CAI Cluster | 3x RTX 3050 Ti (4GB each) | ~60W x 3 = **180W** | "The capital of France is Paris..." |
| CAI Cluster | 4x GTX 1060 (6GB each) | ~40W x 4 = **160W** | "The capital of France is Paris..." |
...
This is what makes CAI unique: **it's not just about making large models accessible to low-end hardware — it's about making inference more energy-efficient by design.**

> **Important:** CAI's strongest energy-saving advantage still comes from 2+ distributed nodes. However, CAI now also supports single-machine oversized-model execution using layer streaming, adaptive precision, and OOM-guarded degradation when VRAM is insufficient.

CAI's built-in energy benchmarking proves this. Run `python cai_cli.py benchmark --mode both` and CAI will:
...
- **Preview splits** before deploying: `python cai_cli.py partition --model microsoft/phi-2 --num-nodes 3`
...
- **Models exceeding VRAM** — CAI can now run models that don't fit in total cluster GPU memory by spilling to RAM and disk.
...
- **One-command Docker build**: `python cai_cli.py build` builds all images (chunk, gateway, monitor).
- **One-command weight preparation**: `python cai_cli.py prepare --model <name> --num-chunks 3` downloads model, splits weights, and saves per-chunk files.
...
CAI supports optional **4-bit (NF4) and 8-bit (INT8) quantization** via bitsandbytes to further reduce memory usage per chunk:

```bash
# Run with 4-bit quantization
python cai_cli.py run --model microsoft/phi-2 --prompt "Hello" --quantize 4bit

# Prepare quantized chunk weights for K8s deployment
python cai_cli.py prepare --model microsoft/phi-2 --num-chunks 3 --quantize 8bit
```
...
|---------|-------------|
| `cai_cli.py run` | Generate text with a distributed HuggingFace model |
| `cai_cli.py run --offload` | Run with CPU/disk offloading for oversized models |
| `cai_cli.py scan` | Show available GPU/CPU/RAM on your machine or cluster |
| `cai_cli.py partition` | Preview how a model would be split across N nodes |
| `cai_cli.py benchmark` | Run energy benchmarking (local, K8s, or both) |
| `cai_cli.py benchmark --sampling-rate 0.1` | Benchmark with 100ms GPU sampling |
| `cai_cli.py benchmark --hf-model <name>` | Benchmark a HuggingFace model with energy monitoring |
| `cai_cli.py dashboard` | Launch the Streamlit visualization dashboard |
| `cai_cli.py build` | Build Docker images for chunk/gateway/monitor |
| `cai_cli.py prepare` | Download model, chunk weights, save for K8s deployment |

### Next-Generation Commands

| Command | What It Does |
|---------|-------------|
| `cai_cli.py autotune` | Auto-tune configuration for optimal performance |
| `cai_cli.py speculative` | Run with speculative decoding (faster inference) |
| `cai_cli.py hybrid` | Run with hybrid parallelism (tensor + pipeline) |
| `cai_cli.py placement` | Generate intelligent placement plan |
| `cai_cli.py energy-loop` | Start adaptive predictive energy control loop with safety guardrails |
| `cai_cli.py fault-tolerant` | Run with fault-tolerant pipeline |
| `cai_cli.py plugins` | List and manage plugins |

### Phase 25 Advanced Algorithm Commands

| Command | What It Does |
|---------|-------------|
| `cai_cli.py fcim` | FCIM worker selection analysis and reports |
| `cai_cli.py adsa` | ADSA adaptive scheduling with multiple policies |
| `cai_cli.py batch` | Dynamic batch processing configuration |
| `cai_cli.py active-inference` | Active inference controller (non-DRL) |
| `cai_cli.py dfs-scheduler` | DFS scheduler with various pruning strategies |
| `cai_cli.py ilp-scheduler` | ILP/heuristic scheduling optimization |
| `cai_cli.py onnx` | PyTorch to ONNX model conversion |
| `cai_cli.py simulate` | Optimized simulation with approximations |
...
## Where CAI is NOT the Best Tool

CAI solves a specific problem. Here are scenarios where other tools are a better fit:
...
- **Ollama / vLLM** on an RTX 4090 will generate tokens faster than a CAI cluster of 3x RTX 3050 Ti.

However, CAI still has an advantage here: the single RTX 4090 draws ~350-450W while the CAI cluster draws ~180W total — **same output, lower energy bill**. So the trade-off is:

| Priority | Best Choice |
|----------|-------------|
| Fastest tokens per second | Single powerful GPU (Ollama/vLLM) |
| Lowest energy per inference | CAI distributed cluster |
| Same output quality | Both (identical — same model, same weights) |

### 2. You Need Production-Grade Throughput

CAI is designed for **low-end hardware clusters**, not high-throughput production serving. If you need to serve hundreds of concurrent users with low latency:
...
CAI's sequential chunk pipeline (Node A -> Node B -> Node C) adds network round-trips per token. Production systems use tensor parallelism (all GPUs compute simultaneously), which is fundamentally faster.
...
CAI requires Python, PyTorch, Kubernetes, Docker, and multiple dependencies. It's an infrastructure tool, not a consumer app.

### 4. You Don't Have Multiple Machines

CAI's core value is **distributing a model across multiple nodes**. If you only have one PC, CAI's distributed features don't help — **but Phase 22's CPU/disk offloading allows running models that exceed your GPU's VRAM on a single machine** by spilling weights to RAM and disk. For other single-machine alternatives:
...
CAI runs on your **private LAN cluster**. If you don't own the hardware and want to borrow compute from strangers over the internet:

- **Petals** — Peer-to-peer swarm over the public internet. No hardware ownership needed.

CAI assumes you control all nodes, configure Kubernetes, and have a local network. It doesn't work across the internet.

### 6. You Need Fine-Tuning, Not Just Inference

CAI is **inference-only**. It generates text from pre-trained models but cannot train or fine-tune them. For training distributed across multiple machines:
...
CAI targets server/desktop hardware with NVIDIA GPUs. For phones, Raspberry Pi, or embedded devices:
...
CAI now supports **4-bit (NF4) and 8-bit (INT8) quantization** via bitsandbytes. You can run with `--quantize 4bit` or `--quantize 8bit` to reduce memory per chunk. However, if your priority is maximum quantization flexibility with many format options:
...
CAI's quantization is simpler (NF4 or INT8 only), but it combines with distribution — you can quantize AND split across nodes, which none of the above tools do.

---

## Honest Summary

| CAI's Strength | CAI's Weakness |
|----------------|----------------|
...
**CAI is best when:** You have 2+ low-end PCs with small GPUs, you want to run a model that doesn't fit on any single one, you want **lower power consumption** than a single high-end GPU, and you want provable energy metrics. Or: you have a single machine with limited VRAM and want layer streaming plus offloading with graceful degradation. Or: you need advanced features like speculative decoding, hybrid parallelism, or fault tolerance.

**CAI is NOT best when:** You need maximum speed above all else, need production throughput, want zero-setup local chat, or only have one machine where the model already fits comfortably and you do not need streaming/offloading controls.

# CAI: Kubernetes AI Inference Platform — Literature Review

## Executive Summary

This literature review covers distributed LLM inference systems, energy-aware scheduling, and related research in model parallelism, resource allocation, and optimization algorithms relevant to CAI's design and implementation.
...
- **Relevance to CAI:** CAI adopts tensor parallelism within nodes, inspired by Megatron's approach
- **Differences:** Megatron primarily targets training; CAI optimizes for inference energy efficiency
...
- **Relevance to CAI:** Similar KV cache optimization goals; both target commodity hardware
- **Differences:** vLLM single-GPU only; CAI multi-node, energy-aware
...
- **Relevance to CAI:** Alternative distributed approach; explores collaborative inference
- **Differences:** Petals decentralized/P2P; CAI centralized K8s-based cluster
- **Trade-offs:** Petals more resilient but higher latency; CAI predictable SLAs
...
- **Relevance to CAI:** Practical reference for single-machine baseline
- **Differences:** Single-machine only; CAI exploits distributed clusters
...
- **Relevance to CAI:** Tiered memory management strategy similar to CAI's approach
- **Differences:** FlexGen single GPU; CAI distributed multi-node
...
- **Relevance to CAI:** Hybrid parallelism framework similar to CAI's model selection
- **Differences:** Paper-only; CAI production-ready implementation
...
- **Relevance to CAI:** Scheduler design inspiration; distributed execution model
- **Differences:** Ray Serve general-purpose; CAI LLM-specific optimizations
...
- **Relevance to CAI:** Direct methodological alignment with CAI's FCIM/ADSA components.
...
- **Relevance to CAI:** Supports CAI's proportional and capacity-aware layer assignment.
...
- **Relevance to CAI:** Strong conceptual alignment with CAI's active inference controller for adaptive optimization.
...
- **Relevance to CAI:** Reinforces CAI's ILP + heuristic strategy for practical scalability.
...
- **Relevance to CAI:** Motivates CAI's hybrid pipeline/tensor strategy for heterogeneous clusters.
...
- **Relevance to CAI:** Foundational parallelism concept informing CAI's pipeline execution model.
...
- **Relevance to CAI:** Validates system-level optimization as necessary for large-model practicality.
...
- **Relevance to CAI:** Supports mixed-precision KV cache design used in CAI.
...
- **Relevance to CAI:** Aligns with tiered memory and placement-aware cache management.
...
- **Relevance to CAI:** Strengthens rationale for CAI's DFS/pruning and adaptive batching pathways.
...
- **Relevance to CAI:** Useful direction for future decode-stage optimization in CAI's generation loop.
...
- **Relevance to CAI:** Expands CAI's evaluation lens from inference-only energy to lifecycle-aware sustainability reporting.
...
- **Relevance to CAI:** Supports CAI's emphasis on decode optimization, memory hierarchy, and quantization.
...
- **Relevance to CAI:** Encourages multi-metric sustainability reporting beyond latency/throughput.
...
- **Relevance to CAI:** Supports CAI's design objective of energy-efficient distributed inference on commodity hardware.
...
- **Relevance to CAI:** Validates CAI's multi-layer approach combining scheduling, parallelism, and memory optimization.
...
- **Relevance to CAI:** Confirms the centrality of system efficiency in modern LLM stacks.
...
- **Relevance to CAI:** Highlights latency reduction opportunities across model, system, and communication layers.
...
- Non-LLM-domain papers in the sheet (for example fluid simulation refinement and semisupervised clustering) were not integrated into this section because they are outside CAI's scope.
...
1. **Edge-LLM Inference With Cost-Aware Layer Allocation and Adaptive Scheduling.**
   ...
   - Relevance: Directly maps to CAI's FCIM/ADSA architecture.
...
2. **Adaptive Model Partitioning for Distributed LLM Inference Across Heterogeneous Devices.**
   ...
   - Relevance: Supports CAI's proportional and capacity-driven layer assignment.
...
3. **Large Language Models Inference Offloading and Resource Allocation in Cloud-Edge Computing: An Active Inference Approach.**
   ...
   - Relevance: Aligns with CAI's active inference controller.
...
4. **Optimizing Cloud Service Efficiency With Infrastructure-Aware Scheduling.**
   ...
   - Relevance: Reinforces CAI's ILP + heuristic scheduler design.
...
5. **TPI-LLM: Serving 70B-Scale LLMs Efficiently on Low-Resource Mobile Devices.**
   ...
   - Relevance: Motivates CAI's hybrid parallelism strategy.
...
7. **DeepSpeed: System Optimizations Enable Training Deep Learning Models with Over 100 Billion Parameters.**
   ...
   - Relevance: Validates system-level optimization emphasis in CAI.
...
8. **Edge Intelligence Optimization for LLM Inference with Batching and Quantization.**
   ...
   - Relevance: Supports CAI's DFS/pruning scheduling rationale.
...
9. **Exploring KV Cache Quantization in Multimodal Large Language Model Inference.**
   ...
   - Relevance: Aligns with CAI mixed-precision KV cache optimization.
...
10. **Accelerating LLM Inference via Dynamic KV Cache Placement in Heterogeneous Memory System.**
    ...
    - Relevance: Consistent with CAI tiered memory and placement-aware cache handling.
...
11. **FlashDecoding++Next: High Throughput LLM Inference With Latency and Memory Optimization.**
    ...
    - Relevance: Useful future direction for CAI decode-path acceleration.
...
12. **Accelerating Quantization-Aware Vector Search for Fast Retrieval in Prompt Engineering.**
    ...
    - Relevance: Important for future CAI RAG-oriented deployment paths.
...
13. **The Energy Cost of Artificial Intelligence Lifecycle in Communication Networks.**
    ...
    - Relevance: Extends CAI evaluation from runtime power to lifecycle sustainability.
...
14. **Energy Cost Modelling for Optimizing Large Language Model Inference on Hardware Accelerators.**
    ...
    - Relevance: Supports CAI focus on decode-stage and memory bottleneck optimization.
...
15. **GreenAI: A Comparative Analysis of Environmental Efficiency in LLM-Generated Code.**
    ...
    - Relevance: Encourages broader sustainability metrics in CAI reporting.
...
16. **Energy and Policy Considerations for Deep Learning in NLP.**
    ...
    - Relevance: Reinforces CAI's efficiency-first design intent.
...
17. **Achieving Peak Performance for Large Language Models: A Systematic Review.**
    ...
    - Relevance: Confirms the multi-layer optimization direction adopted by CAI.
...
18. **Efficient LLMs Training and Inference: An Introduction.**
    ...
    - Relevance: Frames CAI's practical engineering priorities.
...
19. **Minimizing Response Latency in LLM-Based Agent Systems: A Comprehensive Survey.**
    ...
    - Relevance: Highlights optimization opportunities aligned with CAI roadmap.
...
| **CAI** | **Dist + Energy-aware** | **3-64** | **100ms** | **10 tok/s** | **50-150W** |

---

## 5. Research Gaps Addressed by CAI
...
**CAI Solution:** DEAS algorithm with real-time power monitoring and threshold-based migration  
...
**CAI Solution:** FCIM multi-criteria scoring with Jain's fairness index  
...
**CAI Solution:** Proportional layer assignment (Algorithm 1) adapts to device capacity  
...
**CAI Solution:** Hybrid parallelism with dynamic mode selection  
...
**CAI Solution:** Comprehensive Streamlit dashboard with real-time telemetry  
...
- **Potential Integration:** Could apply within CAI to reduce TTFT
...
- **Future Work:** Extend CAI to support MoE models

#### Retrieval-Augmented Generation (RAG)
- **Status:** Popular pattern (LangChain, LlamaIndex)
- **Relevance:** Adds intermediate retrieval step before generation
- **Challenge:** Interleave retrieval scheduler with CAI inference scheduler
...
- **Future Work:** Experiment with NVlink-accelerated CAI clusters
...
```bibtex
@article{cai_lit_review_2026,
  title={CAI: Kubernetes AI Inference Platform -- Literature Review},
  author={CAI Development Team},
  year={2026},
  note={GreenCluster-AI-CAI: https://github.com/misbah7172/GreenCluster-AI-CAI}
}
```

### How to Cite CAI Methodology

```bibtex
@article{cai_methodology_2026,
  title={CAI: Kubernetes AI Inference Platform -- Methodology (13-Section Format)},
  author={CAI Development Team},
  year={2026},

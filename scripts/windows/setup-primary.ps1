...
Ensure-CaiAdministrator -ScriptPath $PSCommandPath -RemainingArgs @('-ServerIp', $ServerIp, '-Distro', $Distro, '-TokenPath', $TokenPath, '-RepoUrl', $RepoUrl, '-Workspace', $Workspace)
Ensure-CaiWslAndDistro -Distro $Distro

$pythonScript = Convert-CaiWindowsPathToWsl -Path (Join-Path $scriptDir '..\bootstrap\setup_primary.py')
...
$token = Get-CaiWslToken -TokenPath $TokenPath -Distro $Distro
...
Write-Host ''
Write-Host 'CAI primary node is ready.'
